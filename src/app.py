...
# f = open("corp_graphing_list.txt","r",encoding='utf-8')
...
for item_name in corp:
    ...
    try:
        ...
        stock_price.append(dic[datetime.datetime.today().strftime('%Y-%m-%d')])'''
    ...
    for i in range(len(abnormal_search)-6):
        ratio.append(price_difference[i]/abnormal_search[i])
        product.append(price_difference[i] * abnormal_search[i])
    ...
    pl.show()'''
                ...
                for i in range(len_ext):
                    '''if err_mean - 0.5*var_mean <= (abs(extrapolation[i]-iffted_abnormal_search[i])) and (abs(extrapolation[i]-iffted_abnormal_search[i])) <= err_mean + 0.5*var_mean:
         err += 1'''
                ...
                var_mean = (var_mean / len(day)) ** 0.5
                for i in range(1, len_ext):
                    '''if abs((iffted_abnormal_search[i]-iffted_abnormal_search[i-1])-(extrapolation[i]-extrapolation[i-1])) <= mean + 0.5*var_mean and abs((iffted_abnormal_search[i]-iffted_abnormal_search[i-1])-(extrapolation[i]-extrapolation[i-1])) >= mean - 0.5*var_mean:
         err += 1'''
                ...
                '''
    nerr=0
    for i in range(int(len_ext/2)):
        nerr += (extrapolation[i]-iffted_abnormal_search[i])*(extrapolation[i]-iffted_abnormal_search[i])
    nerr/= int(len_ext/2)
    err+=nerr

    nerr=0
    for i in range(int(len_ext/2),len_ext):
        nerr += (extrapolation[i]-iffted_abnormal_search[i])*(extrapolation[i]-iffted_abnormal_search[i])
    nerr/= len_ext-int(len_ext/2)
    err+=nerr
  ...
  for i in range(len(nw_iffted_abnormal_search)):
      mean += nw_iffted_abnormal_search[i]
  mean = mean / len(nw_iffted_abnormal_search)
  for i in range(len(nw_iffted_abnormal_search)):
      var += (mean - nw_iffted_abnormal_search[i])**2
  var = (var/len(nw_iffted_abnormal_search))**0.5
  for i in range(len(nw_iffted_abnormal_search)):
      if abs(nw_iffted_abnormal_search[i]) > mean + var:
          if nw_iffted_abnormal_search[i] > 0:
              nw_iffted_abnormal_search[i] = mean + var
          else:
              nw_iffted_abnormal_search[i] = mean - var'''
...
def show(a=3):
    plt.title(item_name)
    if (a == 0):
        plt.plot(day, abnormal_search, price_difference)
    elif (a == 1):
        plt.plot(day, abnormal_search, price_difference1)
    elif (a == 2):
        plt.plot(day, abnormal_search, price_difference2)
    elif (a == 3):
        plt.plot(day, abnormal_search, stock_price)
    plt.show()